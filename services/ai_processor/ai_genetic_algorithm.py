...
class GeneticAlgorithmEngine:
    ...
    def create_random_gene(self, gene_id: str) -> StrategyGene:
        ...
        return StrategyGene(
            id=gene_id, parameters=parameters, generation=self.generation
        )
    ...
    async def evaluate_population(self):
        ...
        print(
            f"📈 Average fitness: {np.mean([g.fitness for g in self.population]):.4f}"
        )
    ...
    def apply_strategy(
        ...
        data["rsi"] = self.calculate_rsi(
            data["close"], parameters.get("rsi_period", 14)
        )
        data["sma_short"] = (
            data["close"].rolling(window=parameters.get("sma_short", 10)).mean()
        )
        data["sma_long"] = (
            data["close"].rolling(window=parameters.get("sma_long", 50)).mean()
        )
        ...
        data["macd_signal"] = (
            data["macd"].rolling(window=parameters.get("macd_signal", 9)).mean()
        )
        data["volume_sma"] = (
            data["volume"].rolling(window=parameters.get("volume_sma", 20)).mean()
        )
        ...
        for i in range(100, len(data)):  # Start after enough data for indicators
            ...
            if position is None:  # No position
                ...
                stop_loss = position["entry_price"] * (
                    1 - parameters.get("stop_loss", 0.05)
                )
                take_profit = position["entry_price"] * (
                    1 + parameters.get("take_profit", 0.10)
                )
    ...
    def calculate_fitness(self, performance: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # Normalize metrics
            total_return_score = min(
                performance["total_return"] * 10, 1.0
            )  # Cap at 10% return
            sharpe_score = min(
                max(performance["sharpe_ratio"] / 2, 0), 1.0
            )  # Normalize Sharpe
            win_rate_score = performance["win_rate"]
            drawdown_score = max(
                0, 1 + performance["max_drawdown"]
            )  # Penalize drawdown
    ...
    async def create_next_generation(self):
        ...
        print(
            f"✅ Created generation {self.generation} with {len(self.population)} individuals"
        )
        ...
        for param_name in self.parameter_ranges.keys():
            if random.random() < self.mutation_rate:
                ...
                if isinstance(min_val, int) and isinstance(max_val, int):
                    ...
                    child_params[param_name] = round(
                        random.uniform(min_val, max_val), 3
                    )
    ...
    async def store_population(self):
        ...
        try:
            ...
            self.redis_client.set(
                "genetic_population", json.dumps(population_data), ex=86400
            )
            ...
            if self.population:
                ...
                self.redis_client.set(
                    "best_strategy", json.dumps(best_gene.to_dict()), ex=86400
                )
