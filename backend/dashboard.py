...
def create_profit_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                color=completed_trades["profit"].apply(lambda x: "green" if x > 0 else "red"),
                size=8,
                symbol="circle",
            ),
            yaxis="y2",
...
def create_strategy_performance_chart() -> go.Figure:
    ...
    try:
        ...
        fig.update_layout(title="Strategy Performance Overview", height=800, showlegend=False)

        return fig

    except Exception as e:
        print(f"Error creating strategy performance chart: {e}")
        fig = go.Figure()
        fig.add_annotation(
