...
class AutoRetrainService:
        ...
        try:
            ...
            for entry in perf_data:
                try:
                    ...
                    accuracy.append(record.get("accuracy", 0))
                    total_return.append(record.get("total_return", 0))
                    sharpe_ratio.append(record.get("sharpe_ratio", 0))
                    win_rate.append(record.get("win_rate", 0))
                    total_trades.append(record.get("total_trades", 0))
                except Exception:
                    continue
            if not accuracy:
                return None
    ...
    async def retrain_model(self, model: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            retrained_model = {
                "id": (f"{model['id']}_RETRAINED_{datetime.now().strftime('%Y%m%d%H%M%S')}"),
    ...
    async def broadcast_model_metrics(self):
        ...
        try:
            ...
            print(f"Error broadcasting model metrics: {e}")
    ...
    async def broadcast_retrain_status(self):
        ...
        try:
            ...
            print(f"Error broadcasting retrain status: {e}")
