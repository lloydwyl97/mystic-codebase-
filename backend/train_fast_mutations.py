...
def save_backtest_result(result: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.info(f"ðŸ’¾ Saved backtest result: {filename}")

    except Exception as e:
        logger.error(f"âŒ Error saving backtest result: {e}")
...
def initialize_live_strategy_training() -> None:
    ...
    try:
        if os.path.exists(STRATEGY_DIR) and os.listdir(STRATEGY_DIR):
            logger.info("ðŸ“‚ Strategy directory already exists - ready for live training")
            return
        ...
        logger.info("ðŸ“‚ Initialized strategy directory for live AI training")
...
def run_training_cycle() -> dict[str, Any]:
    ...
    try:
        ...
        if not strategies:
            logger.info("ðŸ“‚ No strategies found for training")
        ...
        logger.info(f"ðŸš€ Starting training cycle for {len(strategies)} strategies")
        ...
        logger.info(f"âœ… Training cycle complete: {len(strategies)} strategies processed")

        return {"trained": len(strategies), "total": len(strategies)}

    except Exception as e:
        logger.error(f"âŒ Error in training cycle: {e}")
        return {"trained": 0, "total": 0, "error": str(e)}


def main():
    """Main loop for continuous strategy training."""
    logger.info("ðŸš€ Mutation Trainer starting...")
    ...
    while True:
        try:
            ...
            logger.info(f"â° Next training cycle in {TRAIN_INTERVAL} seconds")
            time.sleep(TRAIN_INTERVAL)

        except KeyboardInterrupt:
            logger.info("ðŸ›‘ Mutation Trainer stopped")
            break
        except Exception as e:
