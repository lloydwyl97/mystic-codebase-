...
def render_market_section() -> None:
    ...
    if not candles:
        _st.info("No candles available for the selected symbol/timeframe.")
        return

    # Use the same candle set for both chart and KPIs: last 24h window by timestamps
    try:
        candles_sorted = sorted(candles, key=lambda x: int(x.get("ts", 0)))
    except Exception:
        candles_sorted = candles
    last_ts = int(candles_sorted[-1].get("ts", 0)) if candles_sorted else 0
    # Detect ts unit (ms vs s) and compute 24h threshold accordingly
    is_ms = last_ts > 10**11
    window_ms = 24 * 60 * 60 * 1000
    threshold = last_ts - (window_ms if is_ms else window_ms // 1000)
    windowed = [c for c in candles_sorted if int(c.get("ts", 0)) >= threshold]
    if not windowed:
        windowed = candles_sorted[-1:]

    ts, o, h, l, closes = _to_price_series(windowed)

    left, right = _st.columns([3, 2])
    with left:
        _st.subheader("OHLC — last 24h")
    ...
    with right:
        _st.subheader("KPIs — derived from chart candles (24h)")
        last_price = float(closes[-1]) if closes else 0.0
        base_price = float(closes[0]) if closes else 0.0
        change_24h = ((last_price - base_price) / base_price * 100.0) if base_price else 0.0
        high_24h = max(h) if h else 0.0
        low_24h = min(l) if l else 0.0

        k1, k2 = _st.columns(2)
        with k1:
            _st.metric("Last Price", safe_number_format(last_price, 2))
            _st.metric("24h High", safe_number_format(high_24h, 2))
        with k2:
            _st.metric("24h Change", f"{safe_number_format(change_24h, 2)}%")
            _st.metric("24h Low", safe_number_format(low_24h, 2))

        _st.caption(f"Chart candles: {len(windowed)}  •  Exchange: {exchange}")
